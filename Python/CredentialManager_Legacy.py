...
def save_credentials(usernameKey, username, password, passwordKey):
    """Updates the dictionary with the key and saves it to the file"""
    
    credentials[usernameKey] = base64.b64encode(username)
    credentials[passwordKey] = base64.b64encode(password)
    
    with open(file_path, 'w') as credential_file:
        json.dump(credentials, credential_file, sort_keys=True, indent=4, ensure_ascii=False)
            print("Credentials saved")

def save_credentials(credentials):
    """Convenience method to centrally handle saving the file"""
